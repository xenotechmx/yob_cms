...
//        $object["colony"] = $colony;
    ...
    public function get_last_card_payment(Request $request)
    {
        ...
        if ($app_user->token_card != "") {
            ...
            if ($card_payment["data"] != "" && $card_payment["data"]!=null) {
...
//        $response["colony"] = $colony;
    ...
    public function get_job_title_to_search(Request $request)
    {
        ...
        $items_puesto = array();
        $items_puesto = DB::select(DB::RAW('SELECT job_title FROM jobs WHERE job_title is not null AND status = "publish" AND publish = 1 AND disbaled_at is null AND job_title LIKE "%' . $request->job_title . '%" GROUP BY job_title ORDER BY job_title LIMIT ' . $page . ', ' . $total_items . ' '));
